...
if ($_SERVER["REQUEST_METHOD"] == "POST") { 
    // Get the posted data from the booking form 
    $serviceDate = $_POST['serviceDate']; 
    $serviceTime = $_POST['serviceTime']; 
    $areaSize = isset($_POST['areaSize']) ? intval($_POST['areaSize']) : 36; // Default to 36 if not set
    $services = isset($_POST['services']) && is_array($_POST['services']) ? $_POST['services'] : []; // Ensure it's an array
    $additionalStaffCount = isset($_POST['additionalStaff']) ? intval($_POST['additionalStaff']) : 0; // Get additional staff count 

    // Calculate area cost
    $baseArea = 36;
    $areaCost = 2520; // Base cost for 36 sqm 
    if ($areaSize > $baseArea) { 
        $areaCost += ($areaSize - $baseArea) * 70; // Additional cost for extra sqm 
    } 

    // Calculate addon costs 
    $addonCost = 0; 
    foreach ($services as $service) { 
        list($serviceName, $price) = explode(': ', $service); 
        $addonCost += floatval($price); 
    } 

    // Calculate additional staff cost 
    $additionalStaffCost = $additionalStaffCount * 645; // ₱645 per person 

    // Total price 
    $totalPrice = $areaCost + $addonCost + $additionalStaffCost; // Include additional staff cost 

    // Debugging output
    error_log("Area Size: " . $areaSize);
    error_log("Area Cost: " . $areaCost);
    error_log("Addon Cost: " . $addonCost);
    error_log("Additional Staff Count: " . $additionalStaffCount);
    error_log("Additional Staff Cost: " . $additionalStaffCost);
    ...
    $_SESSION['booking'] = [ 
        ...
        'additionalStaffCount' => $additionalStaffCount // Store additional staff count 
    ]; 
} 
?>
...
<html>
  ...
  <body class="bg-body" data-bs-spy="scroll" data-bs-target="#navbar" data-bs-root-margin="0px 0px -40%" data-bs-smooth-scroll="true" tabindex="0">
    ...
    <section class="container mt-5">
        ...
        <p>Service Date: <?php echo htmlspecialchars($serviceDate); ?></p>
        <p>Service Time: <?php echo htmlspecialchars($serviceTime); ?></p> <!-- Display AM/PM -->
        <p>Area Size: <?php echo htmlspecialchars($areaSize); ?> sqm</p>
        <p>Selected Services: <?php echo htmlspecialchars(implode(', ', $services)); ?></p>
        <p>Additional Staff: <?php echo htmlspecialchars($additionalStaffCount); ?> (₱645 each)</p>
        <p>Total Price: ₱<?php echo number_format($totalPrice, 2); ?></p>
    </section>

    <section class="container mt-5">
        <h3>Please Enter Your Address and Contact Information</h3>
        <form action="finalize_booking.php" method="POST">
            ...
            <div class="mb-3">
                <label for="additionalStaff" class="form-label">Additional Staff (₱645 each)</label>
                <input type="number" class="form-control" id="additionalStaff" name="additionalStaff" value="0" min="0">
            </div>
            <input type="hidden" name="serviceDate" value="<?php echo htmlspecialchars ($serviceDate); ?>">
            <input type="hidden" name="serviceTime" value="<?php echo htmlspecialchars($serviceTime); ?>">
            <input type="hidden" name="timePeriod" value="<?php echo htmlspecialchars($timePeriod); ?>"> <!-- Pass AM/PM -->
            <input type="hidden" name="areaSize" value="<?php echo htmlspecialchars($areaSize); ?>">

...
$bookingSuccess = false; // Flag to check if booking was successful
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $additionalStaffCount = intval($_POST['additionalStaff']);

    // Prepare an SQL statement to prevent SQL injection
    $stmt = $conn->prepare("INSERT INTO bookings (first_name, last_name, address, city, state, pin_code, phone, email, service_date, service_time, area_size, total_price, additional_staff) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
    
    // Bind parameters
    $stmt->bind_param("sssssssssdsii", $firstName, $lastName, $address, $city, $state, $pinCode, $phone, $email, $serviceDate, $serviceTime, $areaSize, $totalPrice, $additionalStaffCount);

    // Execute the statement
    if ($stmt->execute()) {
        $bookingSuccess = true; // Set flag to true if booking was successful
    } else {
        $errorMessage = "Error: " . $stmt->error; // Capture error message
    }
...
<div class="container mt-5">
    ...
    <div class="booking-confirmation">
        ...
        <p class="text-center mb-4">Your booking details are as follows:</p>
        
        <div class="booking-details">
            <p>
                <span class="detail-label">Name:</span>
                <span class="detail-value"><?= htmlspecialchars($firstName) . ' ' . htmlspecialchars($lastName) ?></span>
            </p>
            <p>
                <span class="detail-label">Address:</span>
                <span class="detail-value"><?= htmlspecialchars($address . ', ' . $city . ', ' . $state . ', ' . $pinCode) ?></span>
            </p>
            <p>
            ...
            <p>
                ...
                <span class="detail-value"><?= htmlspecialchars($serviceDate) ?></span>
            </p>
            <p>
                <span class="detail-label">Service Time:</span>
                <span class="detail-value"><?= htmlspecialchars($serviceTime) ?></span>
            </p>
            <p>
                <span class="detail-label">Area Size:</span>
                <span class="detail-value"><?= htmlspecialchars($areaSize) ?> sqm</span>
            </p>
